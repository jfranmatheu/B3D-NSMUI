...
bl_info = {
    "name" : "NewSculptUI_SculptFeatures",
    "author" : "JFranMatheu",
...
icons = {"mirror_icon" : "mirror_icon.png",
         ...
         "colorUnified_icon"  : "colorUnified_icon.png",
            }
...
if addonName != "NewSculptUI_SculptFeatures": # CHANGE THIS
    message = ("\n\n"
        "The name of the folder containing this addon has to be 'NewSculptUI_SculptFeatures'.\n" # CHANGE THIS
        "Please rename it.")
...
class NSMUI_HT_toolHeader_sculpt(Header, UnifiedPaintPanel):
    ...
    ## SCULPT FEATURES BRANCH ONLY
            #activeTool = bpy.context.workspace.tools.from_space_view3d_mode("SCULPT", create=False).idname
        # MESH FILTER // bpy.ops.wm.tool_set_by_id(name="builtin.mesh_filter") // layout.operator("wm.tool_set_by_id").name = builtin.mesh_filter
            if bpy.context.workspace.tools.from_space_view3d_mode("SCULPT", create=False).idname == "mask_filter":
                return
    ...
    ## FOR SCULPT FEATURES BRANCH
    # COLORÇ
            icon = pcoll["colorUnified_icon"]
            layout = self.layout
            split = layout.split()
            col = self.layout
            row = col.row(align=True)
        # COLOR SELECTOR
            row.prop(brush, "color", slider=False, toggle=False, text="")
        # COLOR UNIFIED
            row.prop(ups, "use_unified_color", text="", icon_value=icon.icon_id)
    ...
    # TEXTURES - QUICK SELECTOR
        ...
        # SPACING // SEPARATOR
            ...
            col.label(text="", icon_value=icon.icon_id)
            layout = self.layout
            layout.separator(factor=50.0) # BYBY OLD UI # OK HAVE TO FIX THIS
...
class NSMUI_HT_header_sculpt(MeshButtonsUI, Header):
    ...
    def draw(self, context):
        ...
        if(tool_mode == "SCULPT"):
            ...
            if(bpy.context.space_data.show_region_tool_header == False):
                ...
                row.operator('nsmui.ot_header_tool_toggle', text="", icon_value=icon.icon_id) # id del operador, texto para el botón
                return None
            else:
                if(temp == False):
                    icon = pcoll["arrowUp_icon"]
                    ...
                    icon = pcoll["arrowDown_icon"]
                ## FOR SCULPT FEATURES BRANCH # COMMENTED
                #row.operator('nsmui.ot_header_tool_toggle', text="", icon_value=icon.icon_id)

        ## SCULPT FEATURES BRANCH
        # REMESH PANEL DROPDOWN
