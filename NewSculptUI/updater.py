...
async def do_request():
    ...
    future   = loop.run_in_executor(None, requests.get, 'https://github.com/jfranmatheu/Blender_NewSMUI/raw/master/NewSculptUI.zip')
    response = await future
    object = json.loads(response.text)
    bpy.types.Scene.newVersion = object[0]["tag_name"]
    for mod in addon_utils.modules():
        if mod.bl_info.get("name") == "NewSculptUI":
...
class Update(bpy.types.Operator):
    bl_idname = "nsmui.update"
    bl_label  = "Update NewSculptUI"
    ...
    def execute(self, context):
        #Download
        download_path = os.path.join(self.tmp.name, "NewSculptUI.zip")
        ...
        #Move the NewSculptUI folder to the root temporary directory
        os.rename(os.path.join(self.tmp.name, "tmp_extract", "NewSculptUI"), os.path.join(self.tmp.name, "NewSculptUI"))
        shutil.rmtree(os.path.join(self.tmp.name, "tmp_extract"))
        #Replace
        final_path = os.path.join(bpy.utils.resource_path('USER'), "scripts", "addons", "NewSculptUI")
        print(final_path)
        go = True
        if go:
            if os.path.exists(final_path):
                if os.path.isdir(final_path):
                    ...
                    os.rename(os.path.join(self.tmp.name, "NewSculptUI"), final_path)
